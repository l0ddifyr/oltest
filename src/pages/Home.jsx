        ...
        const channel = supabase.channel(`ols-\${tastingId}`)
            .on("postgres_changes", { event: "UPDATE", schema: "public", table: "tastings", filter: `id=eq.\${tastingId}` }, async (payload) => {
                if (payload.new.current_beer_no !== currentBeerNo) {
                    //   setCurrentBeerNo(payload.new.current_beer_no); // This might cause loop if not careful, but okay here.
                    //   Better to trigger reload
                    const newNo = payload.new.current_beer_no;
                    setCurrentBeerNo(newNo);
                    // We rely on useEffect[currentBeerNo] to reload ratings
                }
            })
            .subscribe((s) => {
                setLiveStatus(s === "SUBSCRIBED" ? "LIVE ✓" : "Offline");
            });
